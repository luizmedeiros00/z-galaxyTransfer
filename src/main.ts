...
const findAviatorByFlyCardNumber = new FindAviatorByFlyCardNumber(
  aviatorRepository,
  airshipRepository
);
...
const saveFlight = new SaveFlight(
  flightRepository,
  airshipRepository,
  routeRepository,
  aviatorRepository,
...
const getFlightsByFlyCardNumber = new GetFlightByFlyCardNumber(
  aviatorRepository,
  flightRepository
);

const aviatorController = new AviatorController(
  saveAviator,
  findAviatorByFlyCardNumber
);
const airshipController = new AirshipController(saveAirship);
const flightController = new FlightController(
  saveFlight,
  getFlightsByFlyCardNumber
);

app.get(
  "/aviators/:flyCardNumber",
...
app.post(
  "/aviators",
  body("name").isString(),
  body("flyCardNumber").isNumeric(),
  body("currentPlanet").isString(),
  async (request: Request, response: Response) => {
...
app.post(
  "/airships",
  body("model").isString(),
  body("yearManufacture").isNumeric(),
  body("fuelCapacity").isNumeric(),
  body("flyCardNumberAviator").isNumeric(),
  async (request: Request, response: Response) => {
...
app.post("/flights", 
  ...
  body("arrivalAt").isISO8601().toDate(),
  body("destinationPlanet").isString(),
  body("airshipId").isNumeric(), 
async (request: Request, response: Response) => {
